...
- 16:9 aspect ratio suitable for presentations`;
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'HTTP-Referer': 'https://slidesai.cloudflare.pages.dev',
        'X-Title': 'SlidesAI',
      },
      body: JSON.stringify({
        model: 'openai/dall-e-3', // Note: This is a paid model. Change to a free one if found.
        messages: [
          {
            role: 'user',
            content: imagePrompt
    ...
    // OpenRouter for DALL-E 3 usually returns a URL or base64 in a different structure
    // We'll try to handle the standard format
    const imageBase64 = data.choices?.[0]?.message?.content || data.choices?.[0]?.message?.images?.[0]?.image_url?.url || data.data?.[0]?.url;

    if (!imageBase64) {
      throw new Error('No image returned from OpenRouter. Please check model compatibility or API credits.');
    }
