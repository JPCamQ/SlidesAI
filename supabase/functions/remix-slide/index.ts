...
serve(async (req) => {
  ...
  try {
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'google/gemini-2.0-flash-exp:free', // Using a vision-capable free model
        messages: [
          {
            role: 'user',
            content: [
    ...
    const imageData = data.choices?.[0]?.message?.content || data.choices?.[0]?.message?.images?.[0]?.image_url?.url || data.data?.[0]?.url;
    if (!imageData) {
